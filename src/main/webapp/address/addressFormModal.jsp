...
<script type="text/javascript">
	function handleAddressForm(addrId, baseurl, index) {
		if (addrId) {
			$.post(baseurl + "/address/getAddressById.action", {
				addrId : addrId
			}, function(result) {
				if (result) {
	...
	function setDefault(addrId, baseurl, e) {
		$.post(baseurl + "/address/setDefaultAddress.action", {
			addrId : addrId
		}, function(result) {
			if (result.setDefault == "success") {
				$(".isDefault").html("");
				$(e.target).parent().siblings("[class='isDefault']").html(
						"默认地址");
			} else {
				$("#msgTitle").html("操作失败");
	...
	function handleAddress(baseurl) {
		...
		$
				.post(
						baseurl + "/address/handleAddress.action",
						$('#addrForm').serialize(),
						function(result) {
							if (result.index) {
								...
								if ($("#addressTable").length > 0)
									newrow = $("#addressTable").find("tr").length;
								if ($("#conaddressTable").length > 0)
									newrow = $("#conaddressTable").find("tr").length;
								var s = "";
								var addrv = addrProvince + " 省" + addrCity
										+ " 市" + addrArea + " 区 " + addrContent
										+ " " + addrReceiver + " 收，电话："
										+ addrTel;
								...
								if ($("#confirmaddrDetails").length > 0)
									s = s
											+ "<tr><td><input type=radio checked name=address value="+addrv+"></td>";

								s = s + "<td><span id='addrPro"+newrow+"'>"
										+ addrProvince + "省 </span>";
								s = s + "<span id='addrCit"+newrow+"'>"
										+ addrCity + "</span> ";
								s = s + "<span id='addrAre"+newrow+"'>"
										+ addrArea + "</span></td>";
								s = s + "<td><span id='addrCon"+newrow+"'>"
										+ addrContent + "</span></td>";
								s = s + "<td><span id='addrRec"+newrow+"'>"
										+ addrReceiver + "</span></td>";
								s = s + "<td><span id='addrTe"+newrow+"'>"
										+ addrTel + "</span></td>";
								s = s + "<td class='isDefault'></td>";
								s = s
										+ "<td><a href='#' onclick=\"handleAddressForm("
										+ result.addrId + ",'" + baseurl + "',"
										+ newrow + "\")>修改</a> |";
								if ($("#addrDetails").length > 0)
									s = s
											+ "<a href='"
											+ baseurl
											+ "/address/delAddress.action?addrId="
											+ result.addrId
											+ "' onclick='return confirm('确定要删除这个收货地址吗？')'>删除</a>| ";

								s = s + "<a href='#' onclick='setDefault("
										+ result.addrId + ",'" + baseurl
										+ "',event)'>设为默认地址</a>";
								s = s + "</td>";
								if ($("#addrDetails").length > 0) {
									if ($("#addressTable").length == 0) {
										...
										c = c
												+ "<thead><tr><th>#</th><th>省-市-区</th><th>街道</th><th>收件人</th><th>电话</th><th>默认地址</th><th>操作</th></tr></thead>";
										c = c + s;
										$("#addrDetails").html(c);
									} else {
...
<div class="modal fade" id="addressFormModal" role="dialog"
	aria-hidden="true" aria-labelledby="myModalLabel">
	<div class="modal-dialog">
		<div class="modal-content">
			<form class="form-inline" role="form" id="addrForm" method="post"
				action="${pageContext.request.contextPath}/address/handleAddress.action">
				...
				<div class="modal-body">
					<input type="hidden" id="index" name="index" value=""> <input
						type="hidden" id="addrId" name="addrId" value="0" />
					<div class="form-group">
						<label for="addrProvince">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;省：</label> <input class="form-control"
							name="addrProvince" id="addrProvince" type="text" placeholder="省"
							required value="" />
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label for="addrContent">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;街&nbsp;&nbsp;&nbsp;道：</label> <input class="form-control"
				...
				<div class="modal-footer">
					<button class="btn btn-primary" type="button"
						onclick="handleAddress('${pageContext.request.contextPath}',index)">确定</button>
